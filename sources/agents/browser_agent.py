...
class BrowserAgent(Agent):
    ...
    async def get_page_text(self, limit_to_model_ctx = False) -> str:
        ...
        page_text = await loop.run_in_executor(self.executor, self.browser.get_text)
    ...
    async def process(self, user_prompt: str, speech_module: type) -> Tuple[str, str]:
        ...
        search_result_raw = await loop.run_in_executor(self.executor, lambda: self.tools["web_search"].execute([ai_prompt], False))
        ...
        while not complete and len(unvisited) > 0 and not self.stop:
            ...
            if len(extracted_form) > 0:
                ...
                fill_success = await loop.run_in_executor(self.executor, lambda: self.browser.fill_form(extracted_form))
            ...
            if speech_module:
                await loop.run_in_executor(self.executor, lambda: speech_module.speak(f"Navigating to {link}"))

            nav_ok = await loop.run_in_executor(self.executor, lambda: self.browser.go_to(link))
            self.search_history.append(link)
            if not nav_ok:
